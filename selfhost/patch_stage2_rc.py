#!/usr/bin/env python3
"""Inject arena-based memory (region GC MVP) into Stage-2 RUNTIME."""
...
/* Region GC MVP: all heap strings/lists live in one arena, freed at process exit. */
...
static void sx_list_push(SxList *l, double v){ if(l->len>=l->cap){ int n=l->cap?l->cap*2:8; double *d=(double*)realloc(l->data,sizeof(double)*(size_t)n); if(!d)exit(1); l->data=d; l->cap=n; } l->data[l->len++]=v; }
...
static void sx_list_drop(SxList *l){ free(l->data); l->data=NULL; l->len=0; l->cap=0; }
...
# Replace RUNTIME="..." contents if present
...
src2 = src[: m.start(1)] + new_runtime + src[m.end(1) :]
path.write_text(src2)
print("Patched Stage-2 RUNTIME with arena GC MVP")
