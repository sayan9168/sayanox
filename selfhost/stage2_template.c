/*
 * Sayanox Stage-2 — always generic lowering
 * hold reassign, struct fields, string/list index, make, file I/O
 */
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
...
static void note_decl(const char *n){
    for(int i=0;i<g_ndecl;i++) if(!strcmp(g_decl_names[i],n)) return;
    if(g_ndecl<128){strncpy(g_decl_names[g_ndecl],n,63);g_decl_names[g_ndecl++][63]=0;}
}
static int is_declared(const char *n){
    for(int i=0;i<g_ndecl;i++) if(!strcmp(g_decl_names[i],n)) return 1; return 0;
}
static int is_list_name(const char *n){
    for(int i=0;i<g_nlists;i++) if(!strcmp(g_list_names[i],n)) return 1; return 0;
}
static int is_struct_type(const char *n){
    for(int i=0;i<g_nstruct;i++) if(!strcmp(g_struct_names[i],n)) return 1; return 0;
}
static int struct_index(const char *n){
    for(int i=0;i<g_nstruct;i++) if(!strcmp(g_struct_names[i],n)) return i; return -1;
}

static void buf_printf(char *buf, size_t *n, size_t cap, const char *fmt, ...){
    if(*n>=cap-1) return;
    va_list ap; va_start(ap,fmt);
    int w=vsnprintf(buf+*n,cap-*n,fmt,ap); va_end(ap);
...
static char *parse_primary(void){
    ...
    if(check(T_IDENT)){
        ...
        /* struct literal Type { a, b } */
        if(is_struct_type(buf) && check(T_LBRACE)){
            int si=struct_index(buf); advance();
            char *tmp=malloc(900);
            snprintf(tmp,900,"((%s){", buf);
            int fi=0;
            if(!check(T_RBRACE)){
                char *e=parse_expr();
                char piece[200];
                if(si>=0 && fi<g_struct_nf[si])
                    snprintf(piece,sizeof(piece),".%s=%s",g_struct_fields[si][fi],e);
                else
                    snprintf(piece,sizeof(piece),"%s",e);
            ...
            expect(T_RBRACE,"}"); strncat(tmp,"})",900-strlen(tmp)-1);
            free(buf); return tmp;
        }
        /* field access .name */
        while(match(T_DOT)){
            ...
            char *n=malloc(768);
            snprintf(n,768,"%s.%s",buf,cur()->text);
            advance(); free(buf); buf=n;
        }
        if(match(T_LBRACK)){
        ...
        if(match(T_LPAREN)){
            ...
            if(!strcmp(buf,"str")) snprintf(call,800,"sx_str(%s)",args);
                ...
                else snprintf(call,800,"sx_len_any(%s)",args);
            }
...
static int looks_string(const char *e){
    if(!e)return 0; if(e[0]=='"')return 1;
    return strstr(e,"sx_str")||strstr(e,"sx_concat")||strstr(e,"sx_read_file")?1:0;
}
static int looks_list(const char *e){ return e&&(strstr(e,"sx_list_new")||strstr(e,"SxList")); }
static int looks_struct(const char *e){
    if(!e)return 0;
    for(int i=0;i<g_nstruct;i++){
        char pat[80]; snprintf(pat,sizeof(pat),"((%s)",g_struct_names[i]);
        if(strstr(e,pat)) return 1;
    }
    return 0;
}
static const char *struct_type_of_expr(const char *e){
    for(int i=0;i<g_nstruct;i++){
        char pat[80]; snprintf(pat,sizeof(pat),"((%s)",g_struct_names[i]);
        if(strstr(e,pat)) return g_struct_names[i];
    }
    return NULL;
}
...
static void parse_stmt(void){
    ...
    if(match(T_STRUCT)){
        ...
        while(!check(T_RBRACE)&&!check(T_EOF)){
            if(check(T_IDENT)){
                if(si<32 && g_struct_nf[si]<8){
                    strncpy(g_struct_fields[si][g_struct_nf[si]],cur()->text,63);
                    g_struct_fields[si][g_struct_nf[si]][63]=0;
                    g_struct_nf[si]++;
                }
                buf_printf(g_types,&g_types_n,BUF_MAX,"  double %s;\n",cur()->text);
                advance(); match(T_COMMA);
            } else advance();
        }
        expect(T_RBRACE,"}");
        buf_printf(g_types,&g_types_n,BUF_MAX,"} %s;\n\n",sname);
        if(si<32) g_nstruct++;
        return;
    }
    ...
    if(match(T_HOLD)){
        ...
        if(is_declared(name)){
            ...
            else if(looks_struct(e)){
                const char *ty=struct_type_of_expr(e);
                emit("%s %s = %s;\n",ty?ty:"double",name,e);
...
static void compile_generic(const char *out_path){
    lex();
    g_funcs_n=0; g_main_n=0; g_types_n=0;
    g_funcs[0]=0; g_mainb[0]=0; g_types[0]=0;
    g_nlists=0; g_nstrs=0; g_ndecl=0; g_nstruct=0;
    g_emit_to_func=0; g_indent=1;
    while(!check(T_EOF)) parse_stmt();

    FILE *o=fopen(out_path,"wb"); if(!o)die("cannot write output");
...
int main(int argc,char **argv){
    ...
    /* ALWAYS generic — no semantic special-case */
    compile_generic(out);
