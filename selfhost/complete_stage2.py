...
# NOTE: ", double " must be a single C string (was broken as ", double ",")
make_replacement = (
    'int string_function=!strcmp(fname,"emit")||!strcmp(fname,"error_record")||!strcmp(fname,"keyword");\n'
    'int param_index=0;\n'
    'if(check(T_IDENT)){\n'
    'const int string_limit=!strcmp(fname,"emit")?2:(!strcmp(fname,"error_record")?1:(!strcmp(fname,"keyword")?1:0));\n'
    'strncat(params,(param_index<string_limit)?"char *":"double ",sizeof(params)-1);'
    'strncat(params,cur()->text,sizeof(params)-1);advance();param_index++;\n'
    'while(match(T_COMMA)){if(!check(T_IDENT))break;'
    'strncat(params,(param_index<string_limit)?", char *":", double ",sizeof(params)-1);'
    'strncat(params,cur()->text,sizeof(params)-1);advance();param_index++;}}\n'
    'expect(T_RPAREN,")");'
)
