...
fn check_stmt(stmt: &Stmt, vars: &mut HashMap<String, Ty>, structs: &mut HashMap<String, Vec<String>>, funcs: &mut HashMap<String, (Vec<Ty>, Ty)>, in_fn: bool) -> Result<(), String> {
    match stmt {
        Stmt::Hold { name, value, .. } => {
            ...
            if let Some(prev) = vars.get(name) { if !prev.compatible(&ty) { return Err(format!("type error: cannot reassign `{}` from {} to {}", name, prev.name(), ty.name())); } }
            vars.insert(name.clone(), ty); Ok(())
        }
        Stmt::Assign { name, value } => {
            let ty = infer_expr(value, vars, structs, funcs)?;
            match vars.get(name) {
                None => Err(format!("type error: assignment to undeclared `{}` (use hold first)", name)),
                Some(prev) if !prev.compatible(&ty) => Err(format!("type error: cannot assign {} to `{}` (was {})", ty.name(), name, prev.name())),
                _ => Ok(()),
            }
        }
        Stmt::StructDef { name, fields, .. } => { structs.insert(name.clone(), fields.clone()); Ok(()) }
        Stmt::Make { name, params, body, .. } => {
            let param_tys = vec![Ty::Number; params.len()]; funcs.insert(name.clone(), (param_tys.clone(), Ty::Number));
            let mut local = vars.clone(); for p in params { local.insert(p.clone(), Ty::Number); }
            for s in body { check_stmt(s, &mut local, structs, funcs, true)?; } Ok(())
        }
...
fn infer_expr(expr: &Expr, vars: &HashMap<String, Ty>, structs: &HashMap<String, Vec<String>>, funcs: &HashMap<String, (Vec<Ty>, Ty)>) -> Result<Ty, String> {
    match expr {
        ...
        Expr::StructLit { name, fields } => {
            if let Some(expected) = structs.get(name) { for (fname, _) in fields { if !expected.iter().any(|f| f == fname) { return Err(format!("type error: struct `{}` has no field `{}`", name, fname)); } } }
            for (_, v) in fields { infer_expr(v, vars, structs, funcs)?; } Ok(Ty::Struct(name.clone()))
        }
        Expr::Field { object, field } => {
            let ot = infer_expr(object, vars, structs, funcs)?;
            if let Ty::Struct(sname) = &ot { if let Some(fields) = structs.get(sname) { if !fields.iter().any(|f| f == field) { return Err(format!("type error: struct `{}` has no field `{}`", sname, field)); } } }
            Ok(Ty::Number)
        }
        Expr::Index { array, index } => {
            let at = infer_expr(array, vars, structs, funcs)?; let it = infer_expr(index, vars, structs, funcs)?;
            if !it.compatible(&Ty::Number) { return Err("type error: index must be a number".into()); }
            match at { Ty::List => Ok(Ty::Number), Ty::String => Ok(Ty::String), Ty::Unknown => Ok(Ty::Unknown), other => Err(format!("type error: cannot index {}", other.name())) }
        }
        Expr::Call { name, args } => {
            ...
            if let Some(spec) = stdlib::builtin(name) {
                if args.len() < spec.min_args || args.len() > spec.max_args {
                    let expected = if spec.min_args == spec.max_args { spec.min_args.to_string() } else { format!("{}..{}", spec.min_args, spec.max_args) };
                    return Err(format!("type error: wrong number of arguments for `{}`: expected {}, got {}", name, expected, args.len()));
                }
            }
            if let Some((params, ret)) = funcs.get(name) {
                if params.len() != args.len() { return Err(format!("type error: wrong number of arguments for `{}`: expected {}, got {}", name, params.len(), args.len())); }
                return Ok(ret.clone());
            }
