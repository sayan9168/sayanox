...
impl Parser {
    ...
    fn use_stmt(&mut self) -> Result<Stmt, String> {
        self.advance();
        let path = match &self.peek().kind {
            ...
            _ => {
                return Err(format!(
                    "Expected string path after 'use' at line {} column {}",
                    self.peek().line,
                    self.peek().column
                ))
            }
        ...
        loop {
            if self.match_token(&TokenKind::LParen) {
                ...
                if let Expr::Ident(name) = expr {
                    ...
                    return Err(format!(
                        "Can only call identifiers at line {} column {}",
                        self.previous().line,
