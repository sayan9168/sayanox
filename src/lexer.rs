            ...
            let kind = match ch {
                ...
                '&' => {
                    if self.match_char('&') {
                        ...
                        return Err(self.error_at(start_line, start_column, "expected '&' after '&'"));
                    }
                }
                '|' => {
                    if self.match_char('|') {
    ...
    fn string(&mut self, start_line: usize, start_column: usize) -> Result<TokenKind, String> {
        ...
        while !self.is_at_end() && self.peek() != '"' {
            if self.peek() == '\\' {
                self.advance();
                if self.is_at_end() {
                    return Err(self.error_at(start_line, start_column, "unterminated escape sequence"));
                }
    ...
    fn number(
        ...
        if matches!(self.peek(), 'e' | 'E') {
            ...
            if !self.peek().is_ascii_digit() {
                return Err(self.error_at(
                    start_line,
                    start_column,
                    &format!("invalid exponent in number '{}'", number),
                ));
            }
            while self.peek().is_ascii_digit() {
                number.push(self.advance());
        ...
        if self.peek() == '.' {
            return Err(self.error_at(
                start_line,
                start_column,
                &format!("invalid number '{}': multiple decimal points", number),
            ));
        }
        ...
        if !value.is_finite() {
            return Err(self.error_at(
                start_line,
                start_column,
                &format!("number '{}' is outside the supported range", number),
            ));
        }
...
mod tests {
    ...
    fn lexes_all_current_keywords() {
        ...
        assert_eq!(tokens[9], TokenKind::Use);
        assert_eq!(tokens[10], TokenKind::Export);
    }
